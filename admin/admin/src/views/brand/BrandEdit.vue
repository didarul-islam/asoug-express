<template>
  <!-- form -->
  <div class="card">
    <div class="card-header"><h3 class="mt-1">Edit Brand</h3></div>
    <hr />
    <div class="card-body">
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.$http
        .get(`admin/v1/brand/${this.$route.params.id}`)
        .then((res) => {
          console.log(res.data);

          // Changed Some Code
          this.name = res.data.brand.name;
          this.arabic_name = res.data.brand.arabic_name;
          this.commision = res.data.brand.commision;
          this.image = res.data.brand.image;

          const categories = [];
          res.data.categories.forEach((category) => {
            if (res.data.brand.category_id === category.id) {
    ...
    categoryUpdate() {
      ...
      this.$http
        .post(`admin/v1/update-brand/${this.$route.params.id}`, formData)
        .then((response) => {
          this.$toast({
            component: ToastificationContent,
            props: {
